...
const AddDriver = () => {
  ...
  const [addDriver, { isLoading: isAddDriverLoading }] = useAddDriverMutation();
  const [deleteDriver, { isLoading: isDeleteDriverLoading }] =
    useDeleteDriverMutation();
  const [updateDriver, { isLoading: isUpdateDriverLoading }] =
    useUpdateDriverMutation();
      ...
      <Box m="0.5rem 0.5rem">
        ...
        >
          ...
          <DataGrid
            loading={!data}
            getRowId={(row) => row.id}
            rows={(data?.data?.drivers && data?.data?.drivers) || []}
            columns={columns}
            rowCount={(data?.data?.drivers && data?.data?.drivers?.length) || 0}
