...
class TempBookingScreen extends StatelessWidget {
  const TempBookingScreen({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return  BlocBuilder<BookingCubit,BookingStates>(
        ...
        :  context.read<BookingCubit>().tempList.isEmpty
        ? Expanded(
            ...
            padding: const EdgeInsets.symmetric(horizontal: 18.0),
              child:RefreshIndicator(
                ...
                child: ListView.separated(
                    itemBuilder: (context, index) =>  BookingCard(color: AppColors.yellow,isTemp: true,
                        bookingTripModel:  context.read<BookingCubit>().tempList[index]
                    ),
                    separatorBuilder: (context, index) =>const SizedBox(height: 20) ,
                    itemCount: context.read<BookingCubit>().tempList.length),
              ),
    ),
        ));
